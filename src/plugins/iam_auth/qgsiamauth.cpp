...
void QgsIAMAuth::performLogin()
{
  ...
  webWidget->dynamicCall( "Navigate(const QString&)", QSettings().value( "iamauth/loginurl", "" ).toString() );
...
//        QMessageBox::information( mLoginDialog, tr( "Authentication successful" ), QString( "Url: %1\nCookie: %2" ).arg( baseUrl.toString() ).arg( cookie ) );
        ...
        QStringList cookieUrls = QSettings().value( "iamauth/cookieurls", "" ).toString().split( ";" );

...
void GlobePlugin::initGui()
{
  ...
  mActionToggleGlobe = mQGisIface->findAction( "mAction3D" );
  if ( !mActionToggleGlobe )
  {
...
{
  disconnect( vLayer, SIGNAL( repaintRequested() ), this, SLOT( refreshQGISMapLayer() ) );
...
void GlobePlugin::updateLayers()
{
  if ( mOsgViewer )
  {
    ...
    Q_FOREACH ( QgsMapLayer* mapLayer, mQGisIface->mapCanvas()->layers() )
    {
      ...
      {
        connect( mapLayer, SIGNAL( repaintRequested() ), this, SLOT( refreshQGISMapLayer() ) );

...
void QgsWcsProvider::readBlock( int bandNo, QgsRectangle  const & viewExtent, int pixelWidth, int pixelHeight, void *block )
{
  ...
  if ( mCachedGdalDataset )
  {
    ...
    if ( mFixRotate && width == pixelHeight && height == pixelWidth )
    {
      ...
      CPLErr err = GDALRasterIO( gdalBand, GF_Read, 0, 0, width, height, tmpData, width, height, ( GDALDataType ) mGdalDataType[bandNo-1], 0, 0 );
      Q_UNUSED( err );
    ...
    {
      CPLErr err = GDALRasterIO( gdalBand, GF_Read, 0, 0, pixelWidth, pixelHeight, block, pixelWidth, pixelHeight, ( GDALDataType ) mGdalDataType[bandNo-1], 0, 0 );
      Q_UNUSED( err );
    ...
    else
    {
      ...
      CPLErr err = GDALRasterIO( gdalBand, GF_Read, 0, 0, width, height, block, pixelWidth, pixelHeight, ( GDALDataType ) mGdalDataType[bandNo-1], 0, 0 );
      Q_UNUSED( err );

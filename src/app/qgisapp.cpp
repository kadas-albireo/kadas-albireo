...
void QgisApp::kmlExport()
{
  ...
  if ( kmlExport.exportToFile( d.getFilename(), d.getSelectedLayers(), d.getExportScale(), mapCanvas()->mapSettings() ) )
  {
...
bool QgisApp::saveDirty()
{
  ...
  if ( askThem && QgsProject::instance()->isDirty() && QgsMapLayerRegistry::instance()->count() > 0 )
  {
    ...
    // prompt user to save
    answer = QMessageBox::question( this, tr( "Save?" ),
                                    tr( "Do you want to save the current project? %1" )
                                    .arg( whyDirty ),
                                    QMessageBox::Yes | QMessageBox::Cancel | QMessageBox::No,
                                    hasUnsavedEdits ? QMessageBox::Cancel : QMessageBox::Yes );
...
{
  QgsDebugMsg( QString( "A network request for %1 timed out, any data received is likely incomplete." ).arg( url.host() ) );
#if 0
